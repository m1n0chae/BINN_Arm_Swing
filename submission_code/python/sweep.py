...
def main():
    ...
    ap.add_argument("--model_type", default=None,
                    help="model.type 오버라이드 (예: cnn, transformer, physres_cnn_attn_ablation)")
    ...
    # ablation_mode 오버라이드 (model.physres_cnn_attn.ablation_mode)
    if args.ablation_mode:
        base_cfg.setdefault("model", {}).setdefault("physres_cnn_attn", {})
        base_cfg["model"]["physres_cnn_attn"]["ablation_mode"] = args.ablation_mode
        # 모델 type도 ablation 모듈로 강제 (proposed 는 ablation 모듈 사용)
        base_cfg["model"]["type"] = "physres_cnn_attn_ablation"
        print(f"[override] ablation_mode = {args.ablation_mode}, model.type = physres_cnn_attn_ablation")
