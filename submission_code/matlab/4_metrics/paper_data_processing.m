...
% =========================================================
%  1) Root-folder setup and file discovery
% =========================================================
% clear; clc; close all;
% [user setting] root data folder
%   Points at the OUTPUT_ROOT written by eval_day1_on_full.py /
%   eval_percent_models_on_full.py. It must contain, per model, the
%   Figure3_loso / Figure4_speedloso / Figure5_datasize /
%   Figure6_datasize_sploso folders with the per-stride prediction CSVs.
target_root_folder = "../../../eval_outputs";
% [user setting] stride indices to use ( [] = all )
stride_idx_list = [];
% -------------------------------------------------
...
% -------------------------------------------------
% 1. recursively find every CSV file
% -------------------------------------------------
...
fprintf('Found %d CSV files. Starting sequential processing.\n', n_files);
% =========================================================
%  2) per-file loop
% =========================================================
for i_file = 1:n_files
    ...
    try
        ...
        warning('Failed to read file: %s', full_csv_path); continue;
    end
    ...
    if ~all(ismember(need_vars, string(T.Properties.VariableNames)))
        warning('Skipping (required columns missing): %s', current_csv_name); continue;
    end
        ...
        for c_idx = 1:n_combos
            ...
            for k=1:n_stride
                ...
                try
                    ...
                    warning('failed to access stride_kinematics_arm time data: %s / %s / %s / stride %d', ...
                        subject_id, session_id, day_id, sid);
                    continue;  % this stride stays NaN
                end
                ...
                Fx_Neg_Integral_True_arr(k) = trapz(time_vec, sig_neg);  % kept negative
    ...
    % --- for trueTable: mean/std of the True/Pred values ---
    ...
    % --- for errTable: mean/std of the errors ---
    ...
    % =========================================================
    % (D) Total Calc — mean ± SD of the per-subject means
    % =========================================================
    ...
    % ===== [NEW] Peak/Integral error columns for errTable =====
    ...
    % --- errTable: existing columns + the NEW columns ---
    errTable = table(RowLabel, ...
        ...
        ... % ===== [NEW] Peak & Integral error columns =====
    ...
    fprintf('  >> trueTable saved: %s\n', trueVarName_str+".mat");
    % ===== [END NEW] =====
    
    evalin('base', sprintf('clear %s %s %s', struct_var_name, errorVarName_char, trueVarName_char));
end
fprintf('\nConversion and table generation finished for all CSV files.\n');