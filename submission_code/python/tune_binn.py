...
ap.add_argument("--max_folds", type=int, default=0)       # 0 = use all folds
ap.add_argument("--seed", type=int, default=98)
ap.add_argument("--device", type=str, default="cuda")
ap.add_argument("--out_csv", type=str, default="tune_binn_results.csv")
ap.add_argument("--max_params", type=int, default=150000) # parameter-count cap
args = ap.parse_args()
...
if not bool(dset.get("use_modeling_input", False)):
    raise RuntimeError("Set dataset.use_modeling_input: true in the config so the CoM (modeling) channels enter X.")
...
if not (isinstance(mp, list) and len(mp) > 0 and isinstance(mf, list) and len(mf) > 0):
    raise RuntimeError("dataset.modeling_points_to_use / modeling_fields_to_use are empty (sacrum_dim cannot be determined).")
...
if wrist_dim_cfg <= 0:
    raise RuntimeError("wrist_dim cannot be computed. Check dataset.axes_imu_acc_local / axes_imu_gyro_local.")
...
# ---- Search space ----
# The search space is shrunk conservatively with MAX_PARAMS so that enough
# sampled combinations satisfy params_count <= MAX_PARAMS.
if MAX_PARAMS <= 100000:
    # small-model space targeting up to ~100K parameters
    embed_dim_list = [32, 40, 48]
    attn_heads_list = [1, 2, 4]  # the embed_dim % heads condition is checked below
    ...
    # mid-size model space targeting up to ~150K parameters
    ...
    # beyond that, the original large search space is fine
    embed_dim_list = [80, 96, 112, 128]
    attn_heads_list = [1, 2, 4]  # embed_dim must be divisible by heads
    dropout_list = [0.05, 0.10, 0.15]
...
# training-related hyperparameters are kept as-is
...
for trial in range(1, args.trials + 1):
    ...
    # === resample until params <= MAX_PARAMS ===
    attempt = 0
    while True:
        attempt += 1
        ...
        # build the model once on CPU and check the parameter count
        tmp = BINN(
            in_dim=F, out_dim=C,
        ...
        if attempt >= 300:
            raise RuntimeError(
                f"[TRIAL {trial}] no combination satisfying params<={MAX_PARAMS} was found "
                f"after 300 resamples. Shrink the search space or raise max_params."
            )
    ...
    for fi, val_sub in enumerate(subs, 1):
        ...
        hist = train_collect_nrmse(
            ...
            device=dev,
            log_dir=None,        # tuning runs skip the TensorBoard trace
