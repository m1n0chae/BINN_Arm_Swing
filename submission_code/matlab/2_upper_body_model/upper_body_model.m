...
%  - The chain always spans all three segments (forearm, upper arm, torso);
%    the accumulated CoM-related acceleration is stored under 'sacrum'
% ================================================================
clc; % close all;
...
% segment scaling — speed-specific amplitude ratios derived from Dataset A (Eq. 2).
% The scalar fallbacks apply only when a session is not ss1/ss2/ss3.
ES_SCALE_BY_SESS = struct('ss1',0.6081,'ss2',0.5362,'ss3',0.4945);
SC_SCALE_BY_SESS = struct('ss1',0.1226,'ss2',0.0880,'ss3',0.0680);
ES_SCALE = 0.5362;
SC_SCALE = 0.0880;     % SC = SC_SCALE * WE

% whether to apply the angular offset in the forward pass
APPLY_OFFSET_TO_FORWARD = true;

% === offset mode ===
% Fixed population-mean angular offsets from Dataset A ('manual' mode).
OFFSET_MODE   = 'manual';          % 'auto' | 'off' | 'manual' | 'gravity'
MANUAL_OFF_WE = deg2rad(118.336);  % rad
MANUAL_OFF_ES = deg2rad(89.355);   % rad
MANUAL_OFF_SC = deg2rad(-91.257);  % rad
...
for iS = 1:numel(all_subj)
    ...
    for iSe = 1:numel(all_sess)
        ...
        % speed-specific amplitude ratios (Eq. 2)
        if isfield(ES_SCALE_BY_SESS, sess)
            ES_SCALE = ES_SCALE_BY_SESS.(sess);
            SC_SCALE = SC_SCALE_BY_SESS.(sess);
        end
        ...
        for iD = 1:numel(all_days)
            ...
            for s = s1:s2
                ...
                if ~getLen(vWE) || ~getLen(vES) || ~getLen(vSC), continue; end
