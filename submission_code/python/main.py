...
def print_config_summary(cfg: Dict) -> None:
    ...
    elif mtype == "physres_cnn_attn":
        print(f"  • physres_cnn_attn: {model.get('physres_cnn_attn', {})}")
...
def build_model_from_cfg(model_cfg: Dict, in_ch: int, out_ch: int) -> torch.nn.Module:
    ...
    if mtype == "physres_cnn_attn":
        hp = model_cfg.get("physres_cnn_attn", {}) or {}

        # 반드시 지정해야 안전합니다 (feature split 때문)
        wrist_dim  = int(hp.get("wrist_dim", 0))
        sacrum_dim = int(hp.get("sacrum_dim", 0))
        if wrist_dim <= 0 or sacrum_dim <= 0:
            raise ValueError("[physres_cnn_attn] Please set model.physres_cnn_attn.wrist_dim and sacrum_dim in config.yaml")
    ...
    if mtype == "physres_cnn_attn_ablation":
        # config.yaml에서 파라미터는 편의상 기존 'physres_cnn_attn' 항목을 재사용합니다.
        hp = model_cfg.get("physres_cnn_attn", {}) or {}

        wrist_dim = int(hp.get("wrist_dim", 0))
        sacrum_dim = int(hp.get("sacrum_dim", 0))
        if wrist_dim <= 0 or sacrum_dim <= 0:
            raise ValueError("[physres_cnn_attn_ablation] Please set wrist_dim and sacrum_dim")
...
def main():
    ...
    # ------------------------------------------------------------------
    # [revision] physres 계열: wrist_dim / sacrum_dim 을 dataset 설정에서 자동 유도
    #
    ...
    # ------------------------------------------------------------------
    if cfg.get("model", {}).get("type", "") in ("physres_cnn_attn", "physres_cnn_attn_ablation"):
    ...
    model_cfg_hp = cfg.get("model", {}).get("physres_cnn_attn", {}) or {}
    save_phys_output = bool(model_cfg_hp.get("save_physics_output", False))
    mtype_check = (cfg.get("model", {}).get("type", "cnn")).lower()
    if save_phys_output and "physres" not in mtype_check:
        save_phys_output = False  # physics output은 BINN 계열에서만 가능
    ...
    for seed in seeds:
        ...
        if mtype == "physres_cnn_attn_ablation":
            ablation_mode = cfg.get("model", {}).get("physres_cnn_attn", {}).get("ablation_mode", "proposed")
