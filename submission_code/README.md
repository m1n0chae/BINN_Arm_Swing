...
matlab/
  ...
  2_upper_body_model/
    ...
    perturb_forearm_amplitude.m                  Regenerates the CoM input with a
                                                    scaled forearm angular amplitude
                                                    (Section III-B sensitivity analysis)
  3_dataset_build/
...
python/                       Flat package; all modules import each other by name,
  ...
  models_*.py                 BINN (incl. the ablation modes), CNN, Transformer
  ...
  eval_noise_r_targeted.py    CoM-pathway corruption analysis (supplementary)
  eval_offset_perturbation.py Torso offset perturbation at inference
  eval_amp_perturbation.py    Forearm amplitude perturbation at inference
  make_aux_ablation_table.py  CoM channel-removal statistics and tables
  configs/                    One config per row of Table II, plus the Section III-B configs
...
3. `upper_body_model.m` — apply the three-link model to Dataset B and produce the
   CoM-related acceleration used as the biomechanical input. The script ships with
   the settings used to generate the released `stride_modeling` data
   (speed-specific amplitude ratios, fixed population-mean offsets,
   `INCLUDE_SC = true`, `WRIST_SRC = 'none'`). The original generation run drew
   the segment lengths from a static anthropometry table rather than the
   per-stride marker distances used here, so regenerated files can differ
   marginally from the released ones; the released `.mat` files are the data of
   record.
...
## 4. Section III-B analyses

All commands are run from inside `python/`.

### 4.1 CoM-pathway corruption (supplementary robustness analysis)
...
### 4.4 Forearm amplitude perturbation (Section III-B)

```matlab
AMP_LIST=[1.00,0.90,0.80,0.70];
run('matlab/2_upper_body_model/perturb_forearm_amplitude.m')
```

```bash
PERTURB_DIR=../data/perturb python eval_amp_perturbation.py --runs_root <trained_runs> --out_csv amp.csv
```

Scales the forearm angular amplitude `A_WE` (and, through Eq. 2, `A_ES` and
`A_SC`) before re-solving the kinematics chain, then evaluates the trained model
with only the inference-time CoM input replaced. The scale factor passes through
the cos/sin terms, so the waveform shape genuinely changes — unlike multiplying a
signal by a constant. The 10% reduction corresponds to the underestimation
observed at the slowest walking speed (Section III-A).

### 4.5 Sacrum IMU baseline (Section III-B)
...
FDR correction, applied jointly to the 128 primary GRF-performance comparisons
reported in Tables IV-V and Figs. 7-9. The additional analyses of Section III-B —
the channel-removal comparisons against the proposed model, the comparisons between
the CNN baseline and the models retaining a CoM channel, and the torso-offset and
forearm-amplitude perturbation comparisons — are corrected as separate families,
as are the supplementary corruption analyses (60 baseline-to-corruption and 48
...
The experimental data are not publicly available due to participant privacy and the
terms of the institutional review board approval (KH2023-091) under which they were
collected. They are available from the corresponding author upon reasonable request.
