% ================================================================
% A) DATA preparation: collect every subj/session/day/stride → time-normalise → aggregate
%     + "true" relative accelerations (WE/ES/SC) from double-differentiated joint positions,
%       stored alongside the "model" values for comparison
%     + sacrum decomposition: sacrum_true ≈ wrist_true + WE_true + ES_true + SC_true
%     + OFFSET_MODE 'gravity' uses per-stride vWE.offset_index/offset_rad
%     + ES/SC offsets = es_offset_scaling_factor/sc_offset_scaling_factor * off_we
%     + every patch is excluded from the legends (no automatic data1,2,... entries)
% ================================================================
clc; % close all;
...
% IMU acceleration frame: 'global' | 'local'  (pos_x/z are global, so global is recommended)
ACC_FRAME = 'global';

% offset options
% 'gravity' : per-stride off_we from vWE.offset_index / vWE.offset_rad
OFFSET_MODE   = 'gravity';           % 'auto' | 'off' | 'manual' | 'gravity'
MANUAL_OFF_WE = deg2rad(0.0);     % used when OFFSET_MODE = 'manual' (rad)
MANUAL_OFF_ES = deg2rad(0.0);
MANUAL_OFF_SC = deg2rad(0.0);
% ES/SC offset scaling (multiples of the WE offset)
es_offset_scaling_factor = 0.76;
sc_offset_scaling_factor = -1.2;

APPLY_OFFSET_TO_FORWARD = true;   % whether the offsets enter the forward-pass angles

% selection filters (empty = process everything)
target_subjects = {};             % e.g. {'S002','S003'}
target_sessions = {};        % e.g. {'ss2','ss1'}
target_days     = {};              % e.g. {'Day1','Day3'}
stride_range    = [];             % e.g. [3 10]; empty = every stride of the Day

% time-normalisation samples
Nnorm  = 200;                     % 0-100% resampled to Nnorm points
...
if ~(exist('stride_kinematics_arm','var')==1 && exist('stride_kinematics_arm_vector','var')==1)
    error('stride_kinematics_arm / stride_kinematics_arm_vector are required.');
end
if ~isfield(stride_kinematics_arm,'L_Wrist')
    error('stride_kinematics_arm.L_Wrist is missing.');
end

% ---------------- aggregation buffers (model and truth) ----------------
% (1) model: wrist IMU, inc(WE/ES/SC), cumulative (shoulder), sacrum IMU
M_wrist_x = []; M_wrist_z = [];
...
% (2) truth: true joint accelerations and relative accelerations
...
% (3) composition (decomposition) check: sacrum_true vs wrist_true + inc_WE_true + inc_ES_true + inc_SC_true
...
for iS = 1:numel(all_subj)
    ...
    for iSe = 1:numel(all_sess)
        ...
        for iD = 1:numel(all_days)
            ...
            % vector lengths (WE/ES/SC) + gravity-offset source (vWE)
            haveVec = true;
            ...
            for s = s1:s2
                ...
                % required position/gyro/acc checks
                ...
                % wrist IMU acceleration (frame selection)
                ...
                % segment lengths (mean over this stride)
                ...
                % gyro bias removal + missing-sample interpolation
                idxF = isfinite(gy);
                ...
                % predicted angles: WE (gyro integration) / ES (scaled) / SC (scaled)
                th_we_pred = cumtrapz(t, gy); th_we_pred = th_we_pred - th_we_pred(1);
                th_es_pred = ES_SCALE * th_we_pred;
                th_sc_pred = SC_SCALE * th_we_pred;

                % true angles (sagittal) — no sacrum position, so there is no true SC angle
                ...
                switch lower(OFFSET_MODE)
                    ...
                    case 'auto'
                        ...
                        % ES/SC via the scaling factors (1.0 would give off_es ≈ off_we)
                        off_es = es_offset_scaling_factor * off_we;
                        off_sc = sc_offset_scaling_factor * off_we;

                    case 'gravity'
                        % read the values for stride s from vWE.offset_index / vWE.offset_rad
                        ...
                        if ~isnan(k0) && ~isnan(target_rad)
                            ...
                            % choose the offset so the angle at that index equals target_rad
                        ...
                        % ES/SC as multiples of the WE offset
                        off_es = es_offset_scaling_factor * off_we;
                        off_sc = sc_offset_scaling_factor * off_we;
                ...
                % forward-pass angles (with or without the offsets)
                ...
                % (model) relative-acceleration terms (2-D, same angle for X/Z)
                ...
                % (truth) relative accelerations
                inc_we_tx = ax_e_t - ax_w_t;   inc_we_tz = az_e_t - az_w_t;   % E - W
                inc_es_tx = ax_s_t - ax_e_t;   inc_es_tz = az_s_t - az_e_t;   % S - E

                % sacrum truth (X/Z) = IMU (frames assumed aligned) → "true" shoulder->sacrum relative acceleration
                ...
                % ------------------ time-normalisation (0-1 → Nnorm) ------------------
                phase_src = linspace(0,1,N);

                % model (wrist IMU, inc, cumulative, sacrum IMU)
                ...
                % truth (true joint accelerations)
                ...
                % composition: wrist_true + inc_WE_true + inc_ES_true + inc_SC_true
                sum_comp_x = wtx + iwx_t + esx_t + scx_it;
                sum_comp_z = wtz + iwz_t + esz_t + scz_it;

                % ------------------ append to the aggregation matrices ------------------
                % model
...
% sacrum decomposition agreement
...
% ================================================================
% C) additional analysis: *model* cumulative sum vs *true sacrum* comparison (with nRMSE)
...
% (W_model + WE_model + ES_model) equals the model shoulder acceleration (M_cum).
...
% RMSE of (W_model + WE_model + ES_model) vs Sacrum_true
...
% model cumulative sum + model SC increment
...
% RMSE of (W_model + WE_model + ES_model + SC_model) vs Sacrum_true
...
% max-min range of the true sacrum signal
...
% ------------------ print results ------------------
fprintf('\n--- additional analysis (*model* cumulative-sum error vs true sacrum IMU) ---\n');
...
col_sac_true = [0   0   0  ];       % sacrum TRUE (black)
col_sac_sd   = [0.6 0.6 0.6];       % sacrum TRUE SD patch (grey)

% helper that lightens a colour slightly
lighten = @(c) 0.3 + 0.7*c;

% sacrum model cumulative sum (M_sum_model_x/z and M_sac_x/z defined above)
...
% ---------- 0-50% window nRMSE (sacrum model vs TRUE) ----------
idx_half = ph_pct <= 50;  % indices of the 0-50% phase window
...
% Sacrum TRUE: patch = grey, line = solid black
...
%% Figure 1-1) model comparison: 2-Seg vs 3-Seg vs Sacrum TRUE
% - 2-Seg Model: WE+ES (shoulder prediction) = M_cum
% - 3-Seg Model: WE+ES+SC (sacrum prediction) = M_cum + M_inc_sc
% - Sacrum TRUE: measured sacrum IMU = M_sac
...
xv = [ph_pct, fliplr(ph_pct)];
col_true_sd = [0.7 0.7 0.7];  % TRUE SD patch (light grey)
...
%% Figure 3) decomposition of the true sacrum signal
if any(isfinite(mu_scx)) || any(isfinite(mu_scz))
    ...
    warning('No sacrum IMU — skipping the cumulative figure.');
end
