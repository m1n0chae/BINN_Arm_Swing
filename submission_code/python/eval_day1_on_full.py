...
====================
Evaluate the trained models under RUNS_ROOT on the 100% Day1 data.
Models of a specific subsampling case (case1, case3, ...) can be filtered.

Usage:
  python eval_day1_on_full.py --case 1
  python eval_day1_on_full.py --case 3

Output: <OUTPUT_ROOT>/<model_outdir>/Figure5_datasize/{percent}/<session>.csv
"""
...
_KNOWN_SVARS = [
    # cross-session (longest match first)
...
def parse_variant(name):
    """Find the known session_variant matched at the end of run.name and take the
    __-segment right before it as dvar.
    A suffix (cnn / cnn_with_model / transformer) is fine because dvar always sits
    immediately before svar."""
...
# Paths. Repository-relative defaults, overridable via environment variables.
#   RUNS_ROOT    training-output (runs) root
#   OUTPUT_ROOT  prediction-CSV output location (input of paper_data_processing.m)
#   DATA_DIR     folder with the .mat files written by build_dataset.m
...
def output_csv_path_day1(model: str, dvar: str, svar: str,
    ...
    """
    Layout:
    - 100% (full): <model>/Figure3_loso/<ssX>.csv or Figure4_speedloso/<train_ssX>.csv
    ...
    sess_basename = fname.replace(".csv", "")  # e.g. "ss1", "train_ss1"
    ...
    if pct == "full":
        # 100% LOSO/cross-session: flat file
...
def find_day1_runs(model_root, case_filter=None):
    """Find runs under <model_root>/*/. With case_filter=1, only case1 (full included)."""
    by_var = {}
    ...
    for d in sorted(os.listdir(model_root)):
        ...
        if case_filter is not None:
            if case_filter == 1:
                # case1 → full_day1 or percent*_day1_case1
...
def main():
    ...
    ap.add_argument("--case", type=int, default=1, help="subsampling case to evaluate (1/2/3/4/5); case 1 also covers full")
    ap.add_argument("--include_full", action="store_true",
                    help="also evaluate 100% (full_day1)")
    ap.add_argument("--only_full", action="store_true",
                    help="evaluate only 100% (full_day1), ignoring --case")
    ap.add_argument("--models_filter", nargs="+", default=None,
                    help="evaluate only these models (e.g. cnn transformer proposed)")
    ap.add_argument("--skip_existing", action="store_true",
                    help="skip output CSVs that already exist")
    args = ap.parse_args()
