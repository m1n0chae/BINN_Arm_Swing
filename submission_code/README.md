...
```
matlab/
  1_stride_segmentation/
    Jinsung_Divide_into_stride_Second_with_gyro.m   Stride extraction from wrist gyroscope
                                                    local minima; resampling to T = 101
  2_upper_body_model/
    merged_subject_peak_to_peak_and_offset.m        Dataset A: segment peak-to-peak
                                                    amplitudes and angular offsets
                                                    -> ratios r_ES/WE, r_SC/WE and
                                                    offsets theta_0,i   (Eq. 1-2, Fig. 3)
    jinsung_Modeling_Third.m                        Three-link model; forearm angle from
                                                    gyroscope integration; CoM-relative and
                                                    absolute CoM acceleration (Eq. 3-4, Fig. 4)
    jinsung_com_estimation_with_IMU_total_stride_unnecessary_SC.m
                                                    CoM acceleration estimation, stride-wise
    run_offset_perturbation.m                       Regenerates the CoM input with a
                                                    perturbed torso angular offset
                                                    (Section III-B sensitivity analysis)
  3_dataset_build/
    experiment_integration.m                        Merges GRF / wrist IMU / model output
                                                    into the .mat structs used by Python
    datasets_number_control.m                       Random stride-wise subsampling to
...
1. `Jinsung_Divide_into_stride_Second_with_gyro.m` — segment continuous walking into
   strides using wrist gyroscope local minima; resample each stride to 101 samples.
2. `merged_subject_peak_to_peak_and_offset.m` — derive the population-level amplitude
   ratios and angular offsets from Dataset A.
3. `jinsung_Modeling_Third.m` — apply the three-link model to Dataset B and produce the
   CoM-related acceleration used as the biomechanical input.
4. `experiment_integration.m` — assemble `stride_grf`, `stride_kinematics_arm`, and
   `stride_modeling`.
5. `datasets_number_control.m` — generate the reduced-data variants.
...
|---|---|---|
| `DATA_DIR` | `../data` | `.mat` files written by `experiment_integration.m` |
...
|---|---|---|---|
| `proposed.yaml` | Proposed BINN | `physres_cnn_attn_ablation`, mode `proposed` | **111,788** |
| `ablation_wo_biomech_arch.yaml` | w/o Biomechanics-informed Architecture | `cnn`, CoM channels concatenated at the input | 268,418 |
| `ablation_wo_cross_attention.yaml` | w/o Cross-Attention | `physres_cnn_attn_ablation`, mode `no_attn` | **111,800** |
| `ablation_wo_com_to_grf_mapper.yaml` | w/o CoM-to-GRF Mapper | `physres_cnn_attn_ablation`, mode `attn_only` | **111,732** |
...
run('matlab/2_upper_body_model/run_offset_perturbation.m')
```
...
written by `run_offset_perturbation.m`; it defaults to `../data/perturb`.
