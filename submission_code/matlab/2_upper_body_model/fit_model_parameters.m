%% ===================== Speed-wise P2P & offset analysis =====================
% Prerequisite: the variable stride_kinematics_arm_vector must be in the workspace.
%   Structure:
...
if ~exist('stride_kinematics_arm_vector','var')
    error('stride_kinematics_arm_vector not found. Load stride_kinematics_arm_vector_gyro.mat first.');
end

%% 0) REL / Subject / Session / Speed mapping
...
% reference REL used to enumerate Subject / Session
...
elseif isfield(stride_kinematics_arm_vector, REL_SC)
    ...
    error('stride_kinematics_arm_vector has none of the WE/ES/SC REL fields.');
end

% Subject list: only S*** fields
...
if isempty(subjects)
    error('no S*** subject fields found inside %s.', rel_for_list);
end

% Session list: only ss* fields (ss1-ss3)
...
fprintf('=== analysis targets ===\n');
fprintf('- Subjects: %s ... %s (%d total)\n', subjects{1}, subjects{end}, numel(subjects));
fprintf('- Sessions: %s (%d total)\n\n', strjoin(sessions', ', '), numel(sessions));

% speed mapping (the experiment has sessions ss1-ss3 only)
...
%% 1) collect per-stride P2P / offset (mean) values (WE / ES / SC)
...
for r = 1:numel(REL_LIST)
    ...
    if ~isfield(stride_kinematics_arm_vector, REL)
        fprintf('[SKIP] %s: not present in stride_kinematics_arm_vector\n', REL);
        continue;
    end
    ...
    for iS = 1:numel(subjects)
        ...
        for iSS = 1:numel(sessions)
            ...
            % Day list (Day1, Day2, ...) — strides may also sit directly under SS without a Day level
            ...
            for iD = 1:numel(days)
                ...
                % --- locate the stride angle cells ---
                segCells = [];

                % 1) check node.time.sagittal_angle.pos (cell array)
                ...
                % 2) the node.sagittal_angle.pos (cell array) layout is supported too
                if isempty(segCells)
                ...
                if isempty(segCells)
                    fprintf('[SKIP] %s-%s-%s-%s: no stride sagittal_angle.pos cells\n', REL, S, SS, DY);
                    continue;
                end
                ...
                % --- compute P2P and mean for every stride ---
                for kk = 1:nStride
                    y = segCells{kk};
...
% avoid cell2table errors on empty inputs
...
disp('=== per-stride P2P / offset collection done ===');


%% 2) P2P: speed-wise analysis (WE / ES / SC + ratios)
clc;

%% 2-1) speed-wise P2P mean±SD for WE / ES / SC
...
%% 2-2) P2P ratios: ES/WE, SC/WE, WE/SC (speed-wise)

clc;

% --- per-trial (Subject×Session×Day) mean P2P ---
% ES/WE
...
if ~isempty(T_SC_p2p) && ~isempty(T_WE_p2p)
    ...
    % also compute the inverse ratio WE/SC
...
% --- aggregate speed-wise (mean±SD by Session) ---
...
disp('=== P2P speed-wise analysis done ===');


%% 3) Offset (mean angle): speed-wise analysis (WE / ES / SC + ratios)
clc;

%% 3-1) speed-wise offset mean±SD for WE / ES / SC
...
%% 3-2) Offset ratios: ES/WE, WE/SC (speed-wise)
...
disp('=== offset speed-wise analysis done ===');
