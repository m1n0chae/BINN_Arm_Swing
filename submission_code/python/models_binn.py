...
class BINN(nn.Module):
    ...
    def __init__(
        ...
        # Flag for the *_enc exploratory modes
        self.use_phys_encoder = self.ablation_mode in ('no_physics_enc', 'no_attn_enc')

        # Whether y_phys is concatenated into the regression-head input (non-enc modes only)
        ...
        #   transforms the raw CoM input into embed_dim-dimensional features
        # -------------------------
        if self.use_phys_encoder:
            p_blocks = []
        ...
        # CoM Feature Extractor (only needed in modes that use cross-attention)
        ...
        # -------------------------
        # (3) Cross-Attention (only in modes that use it)
        # -------------------------
        if self.use_sacrum_enc:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 3. CoM Feature Extractor + Cross-Attention (only in the relevant modes)
        if self.use_sacrum_enc:
            # phys_to_attn: feed y_phys into the CoM FE instead of the raw CoM input
    ...
    def get_physics_output(self, x: torch.Tensor) -> torch.Tensor:
        """Return only y_phys, the output of the CoM-to-GRF Converter."""
        sacrum = x[:, :, self.wrist_dim:self.wrist_dim + self.sacrum_dim]
        s = sacrum.transpose(1, 2).contiguous()
        s_sm = self.phys_smooth(s)
        y_phys = self.phys_map(s_sm).transpose(1, 2).contiguous()
